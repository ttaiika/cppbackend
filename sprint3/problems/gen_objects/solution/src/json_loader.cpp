...
// Основная функция загрузки с поддержкой лута
...
model::Game LoadGameLegacy(const std::filesystem::path& json_path) {
    auto result = LoadGame(json_path);
    return std::move(result.game);
}
...
void AddMapsToGame(model::Game& game, 
    ...
    for (const auto& item : maps) {
        AddMapToGame(game, extra_data, item, default_speed, loot_config);
    }
}
...
void AddMapToGame(model::Game& game,
    ...
    if (map_obj.contains("dogSpeed")) {
        default_speed = map_obj.at("dogSpeed").as_double();
    }

    // создали карту
    model::Map map(model::Map::Id{id}, name, default_speed);
...
// добавляем на карту дороги (без изменений)
...
// добавляем на карту здания (без изменений)
...
// добавляем на карту офисы (без изменений)
