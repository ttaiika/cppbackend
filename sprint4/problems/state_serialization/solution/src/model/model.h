...
public:
    ...
    Road(HorizontalTag, geom::Point2D start, geom::Coord end_x) noexcept
        : start_{start}
        , end_{end_x, start.y}
        , m_min_X(std::min<double>(static_cast<double>(start.x), 
                                  static_cast<double>(end_x)) - Road::HALF_WIDTH)
        , m_max_X(std::max<double>(static_cast<double>(start.x), 
                                  static_cast<double>(end_x)) + Road::HALF_WIDTH)
        , m_min_Y(static_cast<double>(start.y) - Road::HALF_WIDTH)
        , m_max_Y(static_cast<double>(start.y) + Road::HALF_WIDTH) {
    }

    Road(VerticalTag, geom::Point2D start, geom::Coord end_y) noexcept
        : start_{start}
        , end_{start.x, end_y}
        , m_min_X(static_cast<double>(start.x) - Road::HALF_WIDTH)
        , m_max_X(static_cast<double>(start.x) + Road::HALF_WIDTH)
        , m_min_Y(std::min<double>(static_cast<double>(start.y), 
                                  static_cast<double>(end_y)) - Road::HALF_WIDTH)
        , m_max_Y(std::max<double>(static_cast<double>(start.y), 
                                  static_cast<double>(end_y)) + Road::HALF_WIDTH) {
    }
    ...
    geom::Point2D GetStart() const noexcept {
        return start_;
    }

    geom::Point2D GetEnd() const noexcept {
        return end_;
    }

    bool Contains(const geom::Point2D& point) const;
...
public:
    ...
    geom::Point2D GenerateRandomPosition() const {
        ...
        return {
            static_cast<geom::Coord>(road.GetStart().x + t * (road.GetEnd().x - road.GetStart().x)),
            static_cast<geom::Coord>(road.GetStart().y + t * (road.GetEnd().y - road.GetStart().y))
        };
    }

    int GetLootValue(size_t loot_type) const {
        if (!extra_data_) {
            return 0;  // Значение по умолчанию
        }
        // Преобразуем ID карты в строку для поиска в extra_data
        std::string map_id = *GetId();  
        return extra_data_->GetLootValue(map_id, loot_type);
    }
    
    void SetExtraData(std::shared_ptr<extra_data::MapExtraData> extra_data) {
...
public:
    ...
    void RestoreSession(const std::shared_ptr<GameSession>& session) {
        sessions_.push_back(session);
    }

    uint32_t GetNextDogId() const {
        return next_dog_id_;
    }

    void SetNextDogId(uint32_t id) {
        next_dog_id_ = id;
    }
