...
void ApiHandler::HandleJoin(const http::request<Body>& req, Send&& send) {
    ...
    try {
        ...
        // Формируем JSON вручную, как во втором файле
...
void ApiHandler::HandleState(const http::request<Body>& req, Send&& send) {
    ...
    // Используем ostringstream с высокой точностью, как во втором файле
    std::ostringstream out;
    out << std::fixed << std::setprecision(18);  // Важно: та же точность!
