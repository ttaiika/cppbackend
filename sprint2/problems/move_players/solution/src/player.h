...
inline bool IsValidFormat(std::string value_) {
    if (value_.size() != 32) return false;
    return std::all_of(value_.begin(), value_.end(), [](unsigned char c){
...
class Player {
public:
    Player(std::unique_ptr<model::Dog> dog, const std::shared_ptr<GameSession>& session);
