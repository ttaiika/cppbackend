#include "players.h"
#include "model.h" // полный доступ к model::Map и Map::Id

std::shared_ptr<Player> Players::AddPlayer(std::unique_ptr<model::Dog> dog, std::shared_ptr<GameSession> session) {
        // Создаем игрока
        auto player = std::make_shared<Player>(std::move(dog), session);
